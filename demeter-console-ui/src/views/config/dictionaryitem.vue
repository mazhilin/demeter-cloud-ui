<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.label" clearable class="filter-item" style="width: 200px;" placeholder="请输入配置Code"/>
      <el-input v-model="listQuery.name" clearable class="filter-item" style="width: 200px;" placeholder="请输入配置名称"/>
      <el-button
        v-permission="['GET /console/dictionary/item/list']"
        class="filter-item"
        type="primary"
        icon="el-icon-search"
      ...
      <el-button
        v-permission="['POST /console/dictionary/item/create']"
        class="filter-item"
        type="primary"
        icon="el-icon-edit"
    ...
    <el-table
      ...
      <el-table-column align="center" label="字典项分类" prop="type">
        <template slot-scope="scope">
          <el-tag :type="scope.row.type ? '1' : '0' ">{{ scope.row.type === '1' ? '业务新增' : '系统新增' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作列表" width="400" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-permission="['POST /console/dictionary/item/update']"
            type="primary"
            size="mini"
            @click="handleUpdate(scope.row)">编辑
          </el-button>
          <el-button
            v-permission="['POST /console/dictionary/item/delete']"
            type="danger"
            size="mini"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form
        ...
        <el-form-item label="数据字典ID" prop="dictionaryId">
          <el-input v-model="dataForm.dictionaryId" placeholder="请输入对应数据字典ID"/>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        dictionaryId: [{ required: true, message: '数据字典ID不能为空', trigger: 'blur' }],
        ...
        message: [{ required: true, message: '字典项描述不能为空', trigger: 'blur' }],
        value: [{ required: true, message: '字典项码值不能为空', trigger: 'blur' }]
      },
  ...
  methods: {
    getList() {
      ...
      listDictionaryItem(this.listQuery).then(response => {
