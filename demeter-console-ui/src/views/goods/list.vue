  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="商品来源" prop="sourceType">
        <template slot-scope="scope" >
          <el-tag :type="scope.row.sourceType ? '1' : '0' ">{{ scope.row.sourceType? 'API接口' : '系统新增' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="danger"
            size="mini"
            @click="handleBound(scope.row)">绑定
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="dialogStatus=='bound'" type="primary" @click="boundTemplateData">确定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        isExpressFree: [{ required: true, message: '请选择是否包邮', trigger: 'blur' }],
        templateId: [{ required: true, message: '运费模板不能为空', trigger: 'blur' }],
        goodsWeight: [{ required: true, message: '货品重量不能为空', trigger: 'blur' }]
      },
      dataForm: {
        id: undefined,
        goodsId: undefined,
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.$router.push({
        path: '/goods/edit',
        query: { page: this.listQuery.page, id: row.id, name: this.listQuery.name, goodsSn: this.listQuery.goodsSn }
      })
    },
    showDetail(detail) {
